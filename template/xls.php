<?php		
	...
	header("Content-Disposition: attachment; filename=stats-$type-$start_date-$final_date.xls");
			...
			if($response){			
				...
				<table border='1'>
					<thead>
						<tr>
							<th>Username</th>
							<th align='center'>$number_of_publications</th>
							<th>$email</th>
					...
					for($i=0; $i<sizeof($users); $i++){
						...
						$html.="
							<tr>
								...
								<td>".normalize_datesXLS($users[$i]['RegisteredDate'])."</td>
							</tr>
						";
						$total+=$users[$i]['Posts'];
					}
	...
	if($type != "friendship"){
        $html.="				
    		<table border='1'>
    			<tr>
    				<td>$number_of_publications $type: $total</td>
    			</tr>
    		</table>
    		</tbody>
    		</table>
